...
public class ReportService {
	...
	public List<Report> getReports(Long renterId, boolean completed, boolean canceled, boolean potential, String period,
		...
		List<Report> reports = getReportsFromRepo(renterId, completed, canceled, potential, period, assetId, fromDate,
				toDate);
	...
	private List<Report> getCombinedReports(Long renterId, String period, Long assetId, String fromDate,
			String toDate) {
		List<Report> reports = reportRepository.getReports(renterId, ReportReservationStatus.ALL, period, assetId,
				fromDate, toDate);
		setGroups(reports, period);
		sortReports(reports);
		return reports;
	...
	private List<Report> getReportsFromRepo(Long renterId, boolean completed, boolean canceled, boolean potential,
		...
		if (completed)
			reports.addAll(reportRepository.getReports(renterId, ReportReservationStatus.COMPLETED, period, assetId,
					fromDate, toDate));

		if (canceled)
			reports.addAll(reportRepository.getReports(renterId, ReportReservationStatus.CANCELED, period, assetId,
					fromDate, toDate));

		if (potential)
			reports.addAll(reportRepository.getReports(renterId, ReportReservationStatus.POTENTIAL, period, assetId,
					fromDate, toDate));
	...
	private void setGroups(List<Report> reports, String period) {
		for (Report r : reports) {
			...
			case "month":
				...
				break;
			case "year":
				r.setGroup(TimeUtils.formatYear(date));
				break;
			case "week":
				r.setGroup(TimeUtils.formatYearMonthDay(date));
				break;
			default:
				r.setGroup(null);
			}
	...
	private void sortReports(List<Report> reports) {
		reports.sort((Report r1, Report r2) -> r1.getGroup().compareTo(r2.getGroup()));
	}
